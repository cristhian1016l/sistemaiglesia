...
class PermissionsController extends Controller
{
    ...
    public function getPermissions(){
        $permisos = DB::select("SELECT doc.NumReg, doc.FecIniReg, doc.FecFinReg, doc.Motivo, doc.PerCon, doc.Estado, c.ApeCon, c.NomCon
                                FROM TabDocumentos doc INNER JOIN TabCon c ON doc.CodCon = c.CodCon WHERE c.EstCon = 'ACTIVO' AND 
                                c.ID_Red = '".Auth::user()->tabredes->ID_RED."'");

        $members = DB::select("SELECT * FROM TabCon WHERE EstCon = 'ACTIVO' AND 
                                ID_Red = '".Auth::user()->tabredes->ID_RED."'");

        $data = ['permissions' => $permisos, 'miembros' => $members];
        return view('liderred.permissions.index', $data);
    }
    ...
    public function getPermissionsAjax(){
        $permisos = DB::select("SELECT doc.NumReg, doc.FecIniReg, doc.FecFinReg, doc.Motivo, doc.PerCon, doc.Estado, c.ApeCon, c.NomCon, c.ID_Red
                                FROM TabDocumentos doc INNER JOIN TabCon c ON doc.CodCon = c.CodCon WHERE c.EstCon = 'ACTIVO' AND c.ID_Red 
                                = '".Auth::user()->tabredes->ID_RED."'");
